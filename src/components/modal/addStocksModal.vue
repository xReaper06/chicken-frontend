          ...
          <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-header">
              ...
              <div class="modal-body">
                <div>
                    <label for="" class="form-label">Edit Quantity And Price</label>
                </div>
                  <input type="number" class="form-control w-50" v-model="newStocks"  placeholder="Enter Quantity">
                  <input type="number" class="form-control w-50" v-model="price"  placeholder="Enter Price">
                <div class="mt-3">
...
import AuthenticationService from '@/service/AuthenticationService.js';
  ...
  const newStocks = ref(0);
  const price = ref(0)
  
  watchEffect(()=>{
    product.value = props.product
      newStocks.value = product.value.quantity
      price.value = product.value.price
  })
  const addNewStocks = async()=>{
    try {
        const response = await AuthenticationService.UpdateProduct({
            prod_id:product.value.id,
            quantity:newStocks.value,
            price:price.value
        })
        if(response){
