            ...
            <table class="table table-responsive table-striped table-hover">
                ...
                <tbody>
                    <tr v-for="order in pendings" :key="order.id">
                        ...
                        <button class="btn btn-secondary" @click="itemShip(order.order_id)">Ship</button>
                        </td>
                    </tr>
                </tbody>
            </table>
...
const getAllItemsPrepare = async()=>{
    try {
        const response = await AuthenticationService.getAllItemsPrepare()
        if(response){
...
const itemShip = async(data)=>{
    try {
        const response = await AuthenticationService.itemShip({
            order_id:data
        })
        if(response){
