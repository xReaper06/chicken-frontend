            ...
            <table class="table table-responsive table-striped table-hover">
                ...
                <tbody>
                    <tr v-for="order in pendings" :key="order.id">
                        ...
                        <button class="btn btn-secondary" @click="itemPrepare(order.order_id)">Prepare</button>
                        </td>
                    </tr>
                </tbody>
            </table>
...
const getAllItemsAccepted = async()=>{
    try {
        const response = await AuthenticationService.getAllItemsAccepted()
        if(response){
            pendings.value = response.data.allItemsAccepted
        }
    } catch (error) {
        console.log(error);
    }
...
const itemPrepare = async(data)=>{
    try {
        const response = await AuthenticationService.itemPrepare({
            order_id:data
        })
        if(response){
